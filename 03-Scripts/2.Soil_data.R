...
darea <- read_excel("01-Data/data_without_coord.xls", sheet = "p_bray")
...
darea <- darea %>% 
  select(district=Partido, code, mean, x0=min, x0.25:x1) %>% 
  as.data.frame()
...
write_csv(x, "01-Data/sim_data_no_coord.csv")

library(terra)
aoi <- vect("01-Data/buenos_aires.shp")
...
aoi$COD_DEPTO_ <- as.numeric(aoi$COD_DEPTO_)
names(aoi)[5] <- "district"

x$district <- as.numeric(x$district)

aoir <- rasterize(aoi, crops, field = "district")
plot(aoir)
aoir_crop <- aoir*crops
aoir_crop[aoir_crop$district==0] <- NaN
plot(aoir_crop)
writeRaster(aoir_crop, "01-Data/land cover/districts_croplands.tif")

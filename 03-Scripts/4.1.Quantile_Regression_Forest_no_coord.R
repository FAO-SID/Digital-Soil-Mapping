...
stratum <- rast("01-Data/land cover/districts_croplands.tif")
names(stratum) <- "stratum"
y <- freq(stratum)
y <- y$value[y$count<300]
stratum[stratum$stratum %in% y] <- NaN
x <- spatSample(x = stratum, 200, method = "stratified", as.df=T, xy=T, replace=F)
table(x$stratum)
